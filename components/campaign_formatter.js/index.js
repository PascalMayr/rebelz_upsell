...
const CampaignFormatter = ({ campaign, setCampaignProperty, isPreviewDesktop }) => {
  ...
  const setStyleProperty = useCallback(
    (value, styleKey) => {
      const currentPreviewStyleKey = isPreviewDesktop ? 'styles' : 'mobileStyles';
      const currentStyleObject = campaign[currentPreviewStyleKey][id];
      const newStyleObject = { ...currentStyleObject, [styleKey]: value };
      setCampaignProperty({ ...campaign[currentPreviewStyleKey], [id]: newStyleObject }, currentPreviewStyleKey);
    },
              ...
              <div id='salestorm-formatter'>
                ...
                <div className="salestorm-formatter-colors">
                  ...
                  <div className='salestorm-formatter-styles'>
                    {styleChoice.value === 'background' && <BackgroundFormatter styles={styles} setStyleProperty={setStyleProperty} />}
                    {styleChoice.value === 'border' && <BorderFormatter styles={styles} setStyleProperty={setStyleProperty} />}
                    {styleChoice.value === 'boxShadow' && <BoxShadowFormatter styles={styles} setStyleProperty={setStyleProperty} />}
                  </div>
                </div>
              </div>
            </Tabs>
