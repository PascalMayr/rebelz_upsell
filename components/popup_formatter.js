...
const PopupFormatter = ({ campaign, setStyleProperty, preview }) => {
  ...
  return (
    <Layout>
      <Layout.Section>
        <Card>
          <Card.Section title="1.) Set Popup Styles">
            <div className='salestorm-banner-formatter-styles-height-width'>
              <TextField
                label="Popup width"
                value={preview === 'desktop' ? campaign.styles.width : campaign.mobileStyles.width}
                onChange={(value) => setStyleProperty(value, 'width')}
                placeholder='width'
              />
              <TextField
                label="Popup height"
                value={preview === 'desktop' ? campaign.styles.width : campaign.mobileStyles.height}
