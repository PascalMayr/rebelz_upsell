...
const SalestormPricingCard = ({title = '', list = [], button = () => <Button primary>Choose this plan</Button>, subtitle = ''}) => {
  return (
      ...
      <Card title={title}>
        ...
        <Card.Section>
          <List className='pricing-list' style={{listStyle: 'none'}}>
            {
              list.map((listItem, index) => <List.Item key={`list-item-${index}`}><span className='salestorm-pricing-card-checkmark'>✓</span>{listItem}</List.Item>)
            }
          </List>
